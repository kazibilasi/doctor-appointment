...
const Appointment = () => {
  ...
  const getAvailableSlots = async () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      // setting end time ot the date with index
            ...
            {docSlots.length &&
              docSlots.map((item, index) => (
                <div
                  ...
                  className={` flex flex-col items-center px-4 py-7 cursor-pointer  rounded-full ${
                    slotIndex === index
            ...
            {docSlots.length &&
              docSlots[slotIndex].map((item, index) => (
                <p onClick={()=>setSlotTime(item.time)} key={index} className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${item.time === slotTime ? 'bg-primary text-white' : 'text-gray-400 border border-gray-300'}`}>
                  {
                    new Date(item.time).toLocaleTimeString().toLowerCase() // Only show the time in lowercase
                  } 
                </p>
              ))}
          </div>
